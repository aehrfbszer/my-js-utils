  ...
  function mainAxios(
    ...
    service.interceptors.request.use(
      (config) => {
        ...
        if (
          token &&
          !noTokenUrls.some((noUrl) => config.url?.includes(noUrl)) &&
      ...
      (error) => {
        ...
        if (error.config.url.includes(refreshTokenUrl.axiosConfig.url)) {
          ...
          const onceAgainRequest = () =>
            mainAxios(axiosConfig, customOptions, count + 1);
...
function httpErrorStatusHandle(error: any, useApiError?: boolean): string {
  ...
  if (useApiError) {
    if (error.response.data?.detail) {
      ...
      typeof error.response.data === "string" && error.response.data.length
    ) {
